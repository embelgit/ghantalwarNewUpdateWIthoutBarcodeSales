<%@page import="com.smt.utility.DatabaseProperties"%>
<%@page import="com.smt.bean.*"%>
...
<%
	...
	if(custName.equalsIgnoreCase("N/A") || custName == null || custName.isEmpty())
	{
		customerName = " ";
	}
	else
	...
	try
	{
		...
		ResultSet rs = stmt.executeQuery("select ItemName, CategoryName, Quantity, SalePrice, ContactNo, OwnerName, TotalAmount, Discount, GrossTotal, Date, totalperitem, TaxAmount,credit_Customer_Name,mobile_No,Gst,Igst,HsnSacNo,payment_mode,BarcodeNo,employee_Name from otherbill where Quantity>0 AND BillNo = "+billno);
		...
		Font font11 = new Font(Font.FontFamily.TIMES_ROMAN, 11, Font.NORMAL, BaseColor.BLACK);		
		...
		float[] columnWidths = { 0.1f, 0.9f, 0.5f, 0.3f, 0.4f };
		...
		float[] columnWidths1 = { 0.1f, 0.9f, 0.5f, 0.3f, 0.4f };
		...
		float[] columnWidths2 = { 0.1f, 0.9f, 0.5f, 0.3f, 0.4f };
		...
		float[] columnWidths3 = { 0.1f, 0.9f, 0.5f, 0.3f, 0.4f };
		...
		float[] columnWidths4 = { 0.1f, 0.9f, 0.5f, 0.3f, 0.4f };
		...
		float[] columnWidths5 = { 0.1f, 0.9f, 0.5f, 0.3f, 0.4f };
		...
		float[] columnWidths6 = { 0.1f, 0.9f, 0.5f, 0.3f, 0.4f };
		...
		float[] columnWidths7 = { 0.1f, 0.9f, 0.5f, 0.3f, 0.4f };
		...
		float[] columnWidths8 = { 0.1f, 0.9f, 0.5f, 0.3f, 0.4f };
		...
		float[] columnWidths9 = { 0.1f, 0.9f, 0.5f, 0.3f, 0.4f };
...
if (a<6)
{
	...
	if(gst1 == 5)
	{
		tax5Amt = tax5Amt+tempTax;
		System.out.println("tax5Amt -------------> "+tax5Amt);
	}
	
	if(gst1 == 12)
	{
		tax12Amt = tax12Amt+tempTax;
		System.out.println("tax12Amt -------------> "+tax12Amt);
	}
	
	if(gst1 == 18)
	{
		tax18Amt = tax18Amt+tempTax;
		System.out.println("tax18Amt -------------> "+tax18Amt);
	}
	
	if(gst1 == 28)
	{
		tax28Amt = tax28Amt+tempTax;
		System.out.println("tax28Amt -------------> "+tax28Amt);
	}	
	...
	double perProdDis = rs.getDouble("Discount");
	double taxperitem = tax / quant;
	double saletotal = ((price / quant)+perProdDis);
	...
	GrandTotal = GrandTotal + Double.parseDouble(totalperitem);
...
table_cell = new PdfPCell(new Phrase("\n\n\n\n\n" + String.format(df.format(totalTaxAmount)) /*TaxAmount*/,
		font13));
...
table_cell = new PdfPCell(new Phrase("Gross Total:"
		+ df.format(GrandTotal), font16));
table_cell.setHorizontalAlignment(Element.ALIGN_CENTER);
table_cell.setBorder(Rectangle.NO_BORDER);
table_cell.setColspan(2);
table.addCell(table_cell);

table_cell = new PdfPCell(new Phrase("|5% Tax Amount:"+ df.format(tax5Amt)+"|12% Tax Amount: "+ df.format(tax12Amt)+"| 18% Tax Amount: "+ df.format(tax18Amt)+"|",font11));
table_cell.setHorizontalAlignment(Element.ALIGN_RIGHT);
table_cell.setBorder(Rectangle.NO_BORDER);
table_cell.setColspan(3);
table.addCell(table_cell);

table_cell = new PdfPCell(new Phrase("28% Tax Amount: "+ df.format(tax28Amt)+"|", font11));
table_cell.setHorizontalAlignment(Element.ALIGN_LEFT);
table_cell.setBorder(Rectangle.NO_BORDER);
table_cell.setPaddingBottom(1);
table_cell.setColspan(2);
...
{	
	...
	table_cell = new PdfPCell(new Phrase("Gross Total:"+ df.format(GrandTotal), font16));
	table_cell.setHorizontalAlignment(Element.ALIGN_CENTER);
	table_cell.setBorder(Rectangle.NO_BORDER);
	table_cell.setColspan(2);
	table1.addCell(table_cell);
