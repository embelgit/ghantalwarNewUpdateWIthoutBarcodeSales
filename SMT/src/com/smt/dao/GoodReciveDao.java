...
public class GoodReciveDao {
	...
	public List getPendingBillPayment(String suppCode, String shopId)
	{
		// TODO Auto-generated method stub
		HibernateUtility hbu = null;
		Session session = null;
		...
		try
		{
			hbu = HibernateUtility.getInstance();
			session = hbu.getHibernateSession();
			Query query = session.createSQLQuery("SELECT gr.PkGoodRecId, gr.pending_bill_payment FROM goodreceive gr where gr.FksuppId = "+suppCode+" AND gr.fkShopId = "+shopId+" ORDER BY gr.FksuppId DESC LIMIT 1;");
			lastBillPending = query.list();
		}
		catch (Exception e)
		{
	...
	public void setPendingBillPaymentToSupp(Double totalBillpending, String suppCode, String shopId)
	{
		...
		{			
			...
			Query query2 = session2.createSQLQuery("UPDATE goodreceive gr SET gr.pending_bill_payment = "+totalBillpending+" WHERE gr.FksuppId = "+suppId+" AND gr.fkShopId = "+shopId);		
	...
	public List<GoodReceive> getSuppWiseStockBetTwoDate(Long suppId, String fDate, String eDate, String shopId)
	{
		HibernateUtility hbu = null;
		Session session = null;
		List<GoodReceive> catList = null;
		...
		try {
			...
			Query query2 = session.createSQLQuery("SELECT gr.barcodeNo, pr.ProductName, ct.Category_Name, gr.Quantity, gr.BuyPrice, gr.Vat, gr.igst, gr.OrignalQuantity, gr.SalePrice, (gr.OrignalQuantity - gr.Quantity), gr.returnQuantity FROM goodreceive gr JOIN categories ct ON gr.fkCatId = ct.pk_category_id JOIN product_reg pr on gr.fkProductId = pr.pkProductNameId WHERE gr.FksuppId ="+suppId+" AND gr.fkShopId = "+shopId+" AND (gr.Date BETWEEN '"+fDate+"' AND '"+eDate+"');");
			List<Object[]> list = query2.list();
			...
			for (Object[] object : list)
			{
				...
				reports.setpReturnQuantity(object[10].toString());
				catList.add(reports);
	...
	public List getBillNo(String shopId)
	{
		...
		try {
			hbu = HibernateUtility.getInstance();
			session = hbu.getHibernateSession();
			Query query = session.createQuery("from GoodReceive WHERE fkShopId = :fkShopId group by billNo");
	...
	public List<PurchaseReportBean> singleDatePurchase(String adate, String shopId)
	{
		// TODO Auto-generated method stub
		HibernateUtility hbu = null;
		Session session = null;
	...
	public List<PurchaseReportBean> suppGstReportBetweenTwoDatesDao(String supplier, Date adate, Date bdate)
	{
		HibernateUtility hbu = null;
		Session session = null;
		List<PurchaseReportBean> catList = null;
		try {
			...
			hbu = HibernateUtility.getInstance();
			session = hbu.getHibernateSession();
			Query query2 = session.createSQLQuery("select g.Date, s.supplier_name, g.BillNo, s.pan_no, g.ItemName, g.BuyPrice, g.OrignalQuantity, g.vat, g.igst, g.HsnSacNo,g.RollSize, g.Discount from goodreceive g left join supplier_details s on g.FksuppId = s.supplier_id where (date BETWEEN :adate AND :bdate) AND g.FksuppId = :supplier");
	...
	public List<PurchaseReportBean> purchaseReportBetweenTwoDatesNonGst(String adate, String bdate)
	{
		HibernateUtility hbu = null;
		Session session = null;
		List<PurchaseReportBean> catList = null;
		...
		try
		{
			...
			catList = new ArrayList<PurchaseReportBean>(0);
			DecimalFormat df = new DecimalFormat("#.##");
			DecimalFormat dfQiM = new DecimalFormat("#.###");
			for (Object[] object : list)
			{				
				PurchaseReportBean reports = new PurchaseReportBean();
				...
				System.out.println("igsttaxAmt" + igsttaxAmt1);
					...
					{
						Qmeter = rSize * qty;
						reports.setQuantityMeter(Double.parseDouble(dfQiM.format(Qmeter)));						
	...
	public List<PurchaseReport> supplierBillWisePurchaseReport(long supplier, String billNo) {
		...
		try {
			...
			for (Object[] object : list)
			{
				...
				reports.setReturnQuantity(object[23].toString());
				reports.setSoldQuantity(object[24].toString());
	...
	public List<PurchaseReport> voucherNoWisePurchaseReportDao(long voucherNo)
	{
		...
		try {
			...
			Double Rsize=0.0;
			System.out.println();
			Query query2 = session.createSQLQuery("select s.BillNo, s.ContactPerson, ct.Category_Name, pr.ProductName, s.HsnSacNo, s.OrignalQuantity, s.BuyPrice, s.Vat, s.igst, s.TaxAmount, s.SalePrice, s.Total, s.ExtraDiscount, s.Expences, s.GrossTotal, s.Date, c.supplier_name,s.RollSize,s.Discount, ((s.BuyPrice*s.OrignalQuantity)*(s.Discount/100)), s.OrignalQuantity, sb.subcat_name, s.returnQuantity, s.soldQuantity from goodreceive s left join supplier_details c ON s.FksuppId=c.supplier_id join categories ct on s.fkCatId=ct.pk_category_id JOIN sub_categories sb on s.fkSubCatId=sb.pk_subcat_id JOIN product_reg pr on s.fkProductId = pr.pkProductNameId where s.voucherNo =:voucherNo");
			...
			for (Object[] object : list)
			{
				...
				reports.setSubCatName(object[21].toString());
				reports.setReturnQuantity(object[22].toString());
				reports.setSoldQuantity(object[23].toString());
	...
	public List<PurchaseReport> barcodeWisePurchaseReport(String barcodeNoOurchase) {
		...
		try {
			...
			double qty = 0.0;
			String Rsize=null;
			Query query2 = session.createSQLQuery("select s.BillNo, ct.Category_Name, pr.ProductName, s.HsnSacNo, s.OrignalQuantity, s.BuyPrice, s.Vat, s.igst, s.TaxAmount, s.Total, s.BarcodeNo, s.Date, c.supplier_name,s.RollSize,s.ExtraDiscount,s.Discount, ((s.BuyPrice*s.OrignalQuantity)*(s.Discount/100)), sb.subcat_name, s.voucherNo, s.SalePrice, s.returnQuantity, s.soldQuantity from goodreceive s left join supplier_details c ON s.FksuppId=c.supplier_id join categories ct on s.fkCatId=ct.pk_category_id JOIN sub_categories sb on s.fkSubCatId=sb.pk_subcat_id JOIN product_reg pr on s.fkProductId = pr.pkProductNameId where s.BarcodeNo = :barcodeNoOurchase");
			...
			for (Object[] object : list)
			{
				...
				reports.setReturnQuantity(object[20].toString());
				reports.setSoldQuantity(object[21].toString());
				catList.add(reports);
	...
	public List<PurchaseReport> singleDatePurchase45(Date adate) {
		...
		try {
			...
			Query query2 = session.createSQLQuery("select s.BillNo, s.ContactPerson, ct.category_name, pr.ProductName, s.HsnSacNo, s.OrignalQuantity, s.BuyPrice, s.Vat, s.igst, s.TaxAmount, s.SalePrice, s.Total, s.ExtraDiscount, s.Expences, s.GrossTotal, s.Date, c.supplier_name, s.RollSize, s.Discount, ((s.BuyPrice*s.OrignalQuantity)*(s.Discount/100)), sb.subcat_name, s.voucherNo, s.returnQuantity, s.soldQuantity from goodreceive s JOIN supplier_details c ON s.FksuppId=c.supplier_id JOIN categories ct on s.fkCatId=ct.pk_category_id JOIN sub_categories sb on s.fkSubCatId=sb.pk_subcat_id JOIN product_reg pr on s.fkProductId = pr.pkProductNameId where s.Date = :adate");
			query2.setParameter("adate", adate);
	...
	public List<PurchaseReport> twoDatePurchase45(Date adate, Date bdate) {
		...
		try {
			...
			Query query2 = session.createSQLQuery("select s.BillNo, s.ContactPerson, ct.category_name, pr.ProductName, s.HsnSacNo, s.OrignalQuantity, s.BuyPrice, s.Vat, s.igst, s.TaxAmount, s.SalePrice, s.Total, s.ExtraDiscount, s.Expences, s.GrossTotal, s.Date, c.supplier_name, s.RollSize, s.Discount, ((s.BuyPrice*s.OrignalQuantity)*(s.Discount/100)), sb.subcat_name, s.voucherNo, s.returnQuantity, s.soldQuantity from goodreceive s left join supplier_details c ON s.FksuppId=c.supplier_id join categories ct on s.fkCatId=ct.pk_category_id JOIN sub_categories sb on s.fkSubCatId=sb.pk_subcat_id JOIN product_reg pr on s.fkProductId = pr.pkProductNameId where s.Date BETWEEN :adate AND :bdate");
			query2.setParameter("adate", adate);
			query2.setParameter("bdate", bdate);
			List<Object[]> list = query2.list();
			catList = new ArrayList<PurchaseReport>(0);
	...
	public List<PurchaseReport> suppliernamewiseAllPurchase(String supplier) {
		...
		try
		{
			...
			for (Object[] object : list)
			{
				...
				reports.setAvailableQuantity(object[5].toString());
				reports.setCatName(object[2].toString());
				reports.setItemName(object[3].toString());
				...
				reports.setReturnQuantity(object[23].toString());
				reports.setSoldQuantity(object[24].toString());
				reports.setBarcodeNo(Long.parseLong(object[25].toString()));
...
/*	public List<SaleReport> employeeNameWiseAndBetweenTwoDates(String fkSupplierId, Date adate, Date bdate) {*/
	...
	// bill no wise stock
		...
		public void setOfferDiscountDao(String disPercentage, String fromBarC, String toBarC, String shopId)
		{
			...
			{				
				hbu = HibernateUtility.getInstance();
				session = hbu.getHibernateSession();
				Query query = session.createQuery("UPDATE GoodReceive SET disPerForBill ="+disPerc+" where barcodeNo BETWEEN "+startBarcode+" AND "+endBarcode+" AND fkShopId = "+shopId);
		...
		public List getTotalAmountBySupp(String supplierId, String shopId)
		{
			com.smt.utility.HibernateUtility hbu = null;
			Session session = null;
			List list = null;
			try
			{
				hbu = HibernateUtility.getInstance();
				session = hbu.getHibernateSession();
				Query query = session.createSQLQuery("SELECT gr.PkGoodRecId, SUM(gr.Total) FROM goodreceive gr where gr.FksuppId = "+supplierId+" AND gr.fkShopId = "+shopId);
				list = query.list();
			}
			catch (Exception e)
			{			
		...
		public List<PurchaseReport> paymentDueDateWiseDao(Date adate, Date bdate) {
			...
			try {
				...
				Query query2 = session.createSQLQuery("select s.BillNo, s.ContactPerson, ct.category_name, pr.ProductName, s.HsnSacNo, s.OrignalQuantity, s.BuyPrice, s.Vat, s.igst, s.TaxAmount, s.SalePrice, s.Total, s.ExtraDiscount, s.Expences, s.GrossTotal, s.Date, c.supplier_name, s.RollSize, s.Discount, ((s.BuyPrice*s.OrignalQuantity)*(s.Discount/100)), sb.subcat_name, voucherNo, paymentDueDate, s.returnQuantity, s.soldQuantity, s.BarcodeNo from goodreceive s left join supplier_details c ON s.FksuppId=c.supplier_id join categories ct on s.fkCatId=ct.pk_category_id JOIN sub_categories sb on s.fkSubCatId=sb.pk_subcat_id JOIN product_reg pr on s.fkProductId = pr.pkProductNameId where s.paymentDueDate BETWEEN :adate AND :bdate");
				query2.setParameter("adate", adate);
				query2.setParameter("bdate", bdate);
				List<Object[]> list = query2.list();
				catList = new ArrayList<PurchaseReport>(0);
				...
				for (Object[] object : list)
				{
					...
					reports.setSubCatName(object[20].toString());
					reports.setVoucherNo(object[21].toString());
					reports.setPaymentDueDate(object[22].toString());
					reports.setReturnQuantity(object[23].toString());
					reports.setSoldQuantity(object[24].toString());
					reports.setBarcodeNo(Long.parseLong(object[25].toString()));
