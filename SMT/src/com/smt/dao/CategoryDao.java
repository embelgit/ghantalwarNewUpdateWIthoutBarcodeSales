...
import com.smt.bean.allSubCategories;
...
public class CategoryDao {
	...
	public List getAllMainCategoriesShopWise(String shopId)
	{
	...
	public List getAllActiveCategoriesShopWise(String shopId)
	{
		Long fkShopId = Long.parseLong(shopId);
		...
		try {
			hbu = HibernateUtility.getInstance();
			session = hbu.getHibernateSession();
			Query query = session.createQuery("from Category WHERE activeYn = 'Y' AND fkShopId = :fkShopId");
			query.setParameter("fkShopId", fkShopId);
			list = query.list();
		} catch (RuntimeException e) {
			Log.error("Error in getAllMainCategories()", e);
	...
	// get All MAin CategoryName In CaTeGory List form
	public List<Category> getAllMAinCaregoryShopWise(String shopId)
	{
	...
	// Delete Category
	public void deleteCategory(Long pk_cat_id)
	{		// TODO Auto-generated method stub
...
Category abc = new Category();
		try
		{
			...
			session = hbu.getHibernateSession();
			transaction = session.beginTransaction();
			Query deleteCatQuery = session.createQuery("update Category set activeYn = :n where pkCategoryId = "+pk_cat_id);
			deleteCatQuery.setParameter("n", n);
			deleteCatQuery.executeUpdate();
			transaction.commit();
	...
	public List getSubCategoriesByRootcategory(String catID)
	{
		...
		try {
			hbu = HibernateUtility.getInstance();
			session = hbu.getHibernateSession();
			Query query = session.createSQLQuery("select pk_subcat_id , subcat_name from sub_categories sb where sb.active_YN = 'Y' AND sb.fk_rootcat_id = "+catID);
			list = query.list();
	...
	// GET All MAIN SUB CATEGORY FROM GOOD RECEIVE FOR GRAPH 
		public List<allSubCategories> getAllMainSubCategoryForGraph() {
			// TODO Auto-generated method stub
			HibernateUtility hbu = null;
			Session session = null;
			List<allSubCategories> subCatList = null;
			try {
				Long k = 0l;
				hbu = HibernateUtility.getInstance();
				session = hbu.getHibernateSession();
				Query query2 = session.createSQLQuery("select sb.pk_subcat_id, sb.subcat_name, ct.category_name from goodreceive gr join sub_categories sb ON gr.fkSubCatId = sb.pk_subcat_id join categories ct ON gr.fkCatId = ct.pk_category_id GROUP BY gr.fkSubCatId");
				List<Object[]> list = query2.list();
				subCatList = new ArrayList<allSubCategories>(0);

				for (Object[] object : list) {
					k++;
					allSubCategories reports = new allSubCategories();
					reports.setSerialnumber(k);
					reports.setPkSubcatId(Long.parseLong(object[0].toString()));
					reports.setSubcatName(object[1].toString()+"("+object[2].toString()+")");
					reports.setCatName(object[2].toString());
					subCatList.add(reports);
